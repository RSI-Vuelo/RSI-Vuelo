...
function Helicopter(props) {
  ...
  function handleSearch(value) {
    ...
    const filteredResults = typeSelected === 'All' ? searchResults : searchResults.filter((r) =>
      r.type === typeSelected &&
  ...
  return (
    <>
      ...
      />
      <br />
      <Button type='primary' className='filterButton' onClick={showDrawer}>Filter Options</Button>
      <Drawer
      ...
      >
        <h3>Type</h3>
        <Radio.Group>
          <Radio value={'All'} style={radioStyle} defaultChecked={true} onClick={() => handleSelected('All')}>
            All
          </Radio>
          {makeItems}
        </Radio.Group>
        <Divider />
        <span><h3>Minimum Capacity Weight</h3><p>{`${capWeight} pounds`}</p></span>
        <Slider defaultValue={capWeight} min={10000} max={50000} onChange={setCapWeight} onAfterChange={handleSlider} />
        <Divider />
        <span><h3>Maximum Crew Members</h3><p>{`${crewMax} ${(crewMax === 1) ? 'person' : 'people'}`}</p></span>
        <Slider defaultValue={crewMax} min={1} max={15} onChange={setCrewMax} onAfterChange={handleSlider} />
        <Divider />
        <span><h3>Minimum Crew Members</h3><p>{`${crewMin} ${(crewMin === 1) ? 'person' : 'people'}`}</p></span>
        <Slider defaultValue={crewMin} min={1} max={15} onChange={setCrewMin} onAfterChange={handleSlider} />
        <Divider />
        <span><h3>Minimum Fuselage Length</h3><p>{`${fuseLength} meters`}</p></span>
        <Slider defaultValue={fuseLength} min={50} max={200} onChange={setFuseLength} onAfterChange={handleSlider} />
        <Divider />
        <span><h3>Minimum Helicopter Height</h3><p>{`${heliHeight} meters`}</p></span>
        <Slider defaultValue={heliHeight} min={5} max={40} onChange={setHeliHeight} onAfterChange={handleSlider} />
        <Divider />
        <span><h3>Minimum Rotor Diameter</h3><p>{`${rotorDiam} meters`}</p></span>
        <Slider defaultValue={rotorDiam} min={10} max={100} onChange={setRotorDiam} onAfterChange={handleSlider} />
        <Divider />
        <h3>Engine Type</h3>
        <Divider />
        <span><h3>Minimum Top Speed</h3><p>{`${maxSpeed} knot${(maxSpeed === 1) ? '' : 's'}`}</p></span>
        <Slider defaultValue={maxSpeed} min={1} max={300} onChange={setMaxSpeed} onAfterChange={handleSlider} />
      </Drawer>
